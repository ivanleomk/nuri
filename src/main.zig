...
fn generateRoutes(init: std.process.Init) !void {
    ...
    const sidebar_html = try tree.generateSidebarHtml(allocator, "/");
    ...
    try buf.appendSlice(allocator, "    _ = current_path;\n");
    try buf.appendSlice(allocator, "    return sidebar_html_root;\n");
    try buf.appendSlice(allocator, "}\n\n");
    try buf.appendSlice(allocator, "const sidebar_html_root: []const u8 = \"");
    
    // Write HTML (escape quotes)
    for (sidebar_html) |c| {
        switch (c) {
            '\\' => try buf.appendSlice(allocator, "\\\\"),
            '"' => try buf.appendSlice(allocator, "\\\""),
